...
### 9. Расхождение имен полей Pydantic (validation_alias) в DSL vs Python

**Симптом:** При импорте DSL Dify возвращает ошибку валидации схемы узла (например, `ValidationError` или `BadRequest`).

**Причина:** Во внутренних Pydantic-схемах Dify бэкенда Python-имена полей отличаются от имен ключей, ожидаемых в JSON/YAML DSL. Бэкенд использует `validation_alias` для маппинга коротких названий из JSON в длинные названия полей Pydantic:
* **Knowledge Retrieval node**:
  - Блок `reranking_model` ожидает ключи `provider` и `model` (в Python-коде `reranking_provider_name` и `reranking_model_name`). Если указать `name` вместо `model` — возникнет ошибка.
  - Блок `embedding_model` аналогично ожидает `provider` и `model` (вместо `embedding_provider_name` и `embedding_model_name`).
* **LLM node**:
  - Конфигурация модели ожидает `provider` и `model` (например, `"provider": "openai"`, `"model": "gpt-4o"`).

**Решение:**
Перед генерацией YAML DSL для узлов всегда проверяйте их реальные схемы в бэкенде:
1. Для узлов, специфичных для базы знаний (например, `knowledge-retrieval`), исходный код схем находится в `~/dify/api/core/workflow/nodes/<type>/entities.py`.
2. Для базовых узлов (LLM, Code, HTTP-Request и т.д.) схемы импортируются из пакета `graphon.nodes`.
3. Всегда используйте короткие имена `provider` и `model` для моделей внутри объектов настроек DSL.
